...
func (s *cubridStmt) Close() error {
	err := gci.Gci_close_req_handle(s.req)
	if err == 0 {
		return nil
	}
...
func (s *cubridStmt) NumInput() int {
	param_cnt := gci.Gci_get_bind_num(s.req)
	if param_cnt < 0 {
...
func (s *cubridStmt) execute(args []driver.Value) (error) {
	...
	_, gciError = gci.Gci_execute(s.req, 0, 0)
	if gciError.Code < 0 {
...
func (s *cubridStmt) bindParam(args []driver.Value) error {
	...
	for i, arg := range args {
		switch arg.(type) {
		case int64:
			res = gci.Gci_bind_param_int(s.req, i + 1, arg, gci.GCI_BIND_PTR)
			if res < 0 {
				return fmt.Errorf("cci_bind_param : %d", res)
			}
		case string:
			res = gci.Gci_bind_param_string(s.req, i + 1, arg, gci.GCI_BIND_PTR)
			if res < 0 {
				return fmt.Errorf("cci_bind_param : %d", res)
			}
		case time.Time:
		...
		case float64:
			res = gci.Gci_bind_param_float(s.req, i + 1, arg, gci.GCI_BIND_PTR)
			if res < 0 {
				return fmt.Errorf("cci_bind_param : %d", res)
			}
		case bool:
