...
func (rows *cubridRows) Columns() []string {
	...
	col_info, _, col_count = gci.Gci_get_result_info(rows.s.req)
	if col_info == nil {
		return nil
	}
	...
	for idx = 1; idx <= col_count; idx++ {
		col_name[idx - 1] = gci.Gci_get_result_info_name(col_info, idx)
	}
...
func (rows *cubridRows) Close() error {
	...
	res = gci.Gci_close_req_handle(rows.s.req)
	if res < 0 {
...
func (rows *cubridRows) Next(dest []driver.Value) error {
	...
	res, err = gci.Gci_cursor(rows.s.req, 1, gci.GCI_CURSOR_CURRENT)
	...
	res, err = gci.Gci_fetch(rows.s.req)
	if res < 0 {
	...
	for i = 1; i <= col_count; i++ {
		columnType = gci.Gci_get_result_info_type(col_info, i)
		
		switch columnType {
		case gci.U_TYPE_CHAR, gci.U_TYPE_STRING, gci.U_TYPE_NCHAR/*, U_TYPE_VARNCHAR*/:
			var data string
			res, data, _ = gci.Gci_get_data_string(rows.s.req, i)
			if res < 0 {
		...
		case gci.U_TYPE_INT, gci.U_TYPE_NUMERIC, gci.U_TYPE_SHORT:
			var data int
			res, data, _ = gci.Gci_get_data_int(rows.s.req, i)
			dest[int(i - 1)] = data
 		case gci.U_TYPE_FLOAT:
			var data float64
			res, data, _ = gci.Gci_get_data_float(rows.s.req, i)
			dest[int(i - 1)] = data
		case gci.U_TYPE_DOUBLE:
			var data float64
			res, data, _ = gci.Gci_get_data_double(rows.s.req, i)
			dest[int(i - 1)] = data
		case gci.U_TYPE_BIT, gci.U_TYPE_VARBIT:
			var data gci.GCI_BIT
			res, data, _ = gci.Gci_get_data_bit(rows.s.req, i)
			dest[int(i - 1)] = data
		case gci.U_TYPE_DATE, gci.U_TYPE_TIME, gci.U_TYPE_TIMESTAMP:
			var data gci.GCI_DATE
			res, data, _ = gci.Gci_get_data_date(rows.s.req, i)
			dest[int(i - 1)] = data
		//case C.CCI_U_TYPE_OBJECT, C.CCI_U_TYPE_RESULTSET:
			//log.Println("cci_a_type_set")
		case gci.U_TYPE_BIGINT:
			var data int64
			res, data, _ = gci.Gci_get_data_bigint(rows.s.req, i)
			dest[int(i - 1)] = data
		case gci.U_TYPE_BLOB:
			var org_data, data gci.GCI_BLOB
			var size int64
			res, org_data, _ = gci.Gci_get_data_blob(rows.s.req, i)
			size = gci.Gci_blob_size(org_data)
			data, err = gci.Gci_blob_read(rows.s.c.con, org_data, 0, size)

			gci.Gci_blob_free(org_data)
			dest[int(i - 1)] = data
		case gci.U_TYPE_CLOB:
			...
			dest[int(i - 1)] = _clob*/
		default:
			if int(gci.Gci_is_collection_type(columnType)) == 1 {
				var data gci.GCI_SET
				var indicator int
				_, data, indicator = gci.Gci_get_data_set(rows.s.req, i)
				if(indicator == -1) {
